...
class FSMAPF_Instance : public Problem
{
private:
...
public:
  ...
  FSMAPF_Instance(const std::string& _instance);
  FSMAPF_Instance(FSMAPF_Instance* P, Config _config_s, Config _config_g,
                  int _max_comp_time, int _max_timestep, std::vector<float>* _radiuses);
  FSMAPF_Instance(FSMAPF_Instance* P, int _max_comp_time);
  ~FSMAPF_Instance();
