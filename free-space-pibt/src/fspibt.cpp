...
Nodes FSPIBT::getNodes(const Agent& ai, const Agent& aj){
    ...
    do {
        if (G->existNode(x + dx, y + dy)) res.push_back(G->getNode(x + dx, y + dy));
        if (G->existNode(x - dy, y + dx)) res.push_back(G->getNode(x - dy, y + dx));
        if (G->existNode(x - dx, y - dy)) res.push_back(G->getNode(x - dx, y - dy));
        if (G->existNode(x + dy, y - dx)) res.push_back(G->getNode(x + dy, y - dx));
        if ((dx - 1)*(dx - 1) + dy*dy > r*r ) --dx;
        else ++dy;
    } while (dx != 0);
